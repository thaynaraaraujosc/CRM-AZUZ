...
/**
 * O formulário como quem responde vê.
 *
 * Isto era o corpo de `/formulario-preview`, uma tela interna que a pessoa do CRM abria pra
 * conferir o próprio formulário. A tela PÚBLICA, a que o link compartilhado abre, simplesmente não
 * existia: o botão "Compartilhar" montava um endereço `/f/<id>` que não correspondia a rota
 * nenhuma no projeto, e quem recebia o link caía num erro do navegador.
 *
 * Em vez de escrever a página pública do zero, o corpo virou este componente e as duas rotas o
 * usam. São a mesma tela porque precisam ser a mesma tela: se fossem duas implementações, a
 * pré-visualização passaria a mentir sobre o que o lead vê, que é justamente o que ela existe pra
 * evitar.
 *
    ...
    return (
      <div className="form-public-page">
        <div className="form-public-card" style={{ background: "#ffffff" }}>
          ...
          <p className="hint">O link pode estar incompleto ou o formulário pode ter sido removido. Peça um link novo pra quem te enviou.</p>
        </div>
      </div>
    );
  }
